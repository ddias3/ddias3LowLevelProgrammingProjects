	...
	*h_sumErrorSquared = 0.0;
	...
	do
	{
		...
		for (int indexKnownData = 0; indexKnownData < LEARNING_DATA_SIZE; ++indexKnownData)
		{
			...
			int movieIndex = h_learningData[indexKnownData].movieIndex;
			int rating = h_learningData[indexKnownData].rating;
			double errorTerm2 = rating - DotProduct(&h_movieFactors[movieIndex * FACTORS], &h_userFactors[userIndex * FACTORS], FACTORS);
		...
		++iterations;
	} while (iterations < 10);//(*h_sumErrorSquared > LEARNING_DATA_SIZE * (ERROR_THRESHOLD * ERROR_THRESHOLD));
